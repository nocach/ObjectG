...
class NotNativeClassGenerator extends Generator {
	...
	public Object generateValue(final GenerationConfiguration configuration, final GenerationContext context) {
		try {
			...
			throw new GenerationException("can't create new instance of " + context.getClassThatIsGenerated().getName(),
					e);
	...
	private void setValuesOnFields(final GenerationConfiguration configuration, final GenerationContext context,
		...
		for (PropertyAccessor each : allPropertiesToSetValues) {
			...
			GenerationContext pushedContext = context
					.push(getMostSpecificFieldType(each, fieldPropertyDesc), resultObj);
	...
	private Object createResultInstance(GenerationConfiguration configuration, GenerationContext context)
	...
	private Object[] createUniqueConstructorArgs(GenerationConfiguration configuration, GenerationContext context,
			Constructor constructor) {
		Object[] constructorParams = new Object[constructor.getParameterTypes().length];
		for (int i = 0; i < constructor.getParameterTypes().length; i++) {
			GenerationContext parameterContext = createContructorArgContext(context, constructor, i);
			constructorParams[i] = generateForHierarchy(configuration, parameterContext);
		}
		return constructorParams;
	}

	private GenerationContext createContructorArgContext(final GenerationContext context, final Constructor constructor,
			final int argIndex) {
		Class paramType = constructor.getParameterTypes()[argIndex];
		final Type genericParameterType = constructor.getGenericParameterTypes()[argIndex];
		GenerationContext parameterContext = context.push(paramType);
		parameterContext.setGenericInformation(genericParameterType);
