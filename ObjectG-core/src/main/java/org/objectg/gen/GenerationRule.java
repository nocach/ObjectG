...
public abstract class GenerationRule<T> implements Comparable<GenerationRule> {
	...
	//we don't want to use "this", because implementations will most likely use it
	private final Object matchedContextStateOwner = new Object();
	private final SessionState<Set<GenerationContext>> matchedContextState =
			GenerationSession.createManagedState(matchedContextStateOwner,
					new SessionStateDescription<Set<GenerationContext>>() {
						@Override
						public Set<GenerationContext> getInitValue() {
							return new HashSet<GenerationContext>();
						}
					});
	 ...
	 */
	public final T getValue(GenerationConfiguration currentConfiguration, GenerationContext context) {
		matchedContextState.get().add(context);
		return getValueInner(currentConfiguration, context);
	}


	protected abstract T getValueInner(GenerationConfiguration currentConfiguration, GenerationContext context);
	...
	/**
	 * @param context
	 * @return if this rule already matched context
	 */
	public boolean didNotMatchAlready(final GenerationContext context) {
		return !matchedContextState.get().contains(context);
	}
}
